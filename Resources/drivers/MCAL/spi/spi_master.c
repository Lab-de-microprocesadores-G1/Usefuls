...
void spiInit(spi_id_t id, spi_cfg_t config)
{
  ...
  {
    portPointers[PIN2PORT(uartPins[id][i])]->PCR[PIN2NUM(uartPins[id][i])] = PORT_PCR_MUX(SPI_PORT_ALTERNATIVE);
  }
  ...
  else
  {
    for (uint8_t i=0; i < len; i++)
    {
      ...
      spi_package_t newPackage = {
    		  .slaves = slave,
			  .frame = message[i],
			  .eoq = ( (i % spiInstances[id].hwFifoSize) == (spiInstances[id].hwFifoSize - 1) || i == len-1) && (i > 0) ? 1 : 0
      };
