...
#define WS2812_FTM_PRESCALE   1                                               // Configuration of the FTM driver
#define WS2812_FTM_MODULO     31                                              // for 800kbps rate needed in WS2812 leds
#define WS2812_HIGH_DUTY      20                                              // Duty value for '1' binit
#define WS2812_LOW_DUTY       10                                              // Duty value for '0' binit
...
typedef struct {
  ...
  uint16_t    frames[2][WS2812_FRAME_SIZE];
...
static void pwmUpdateCallback(uint16_t *frameToUpdate, uint8_t frameCounter)
{
  ...
  for (uint8_t i = 0 ; i < WS2812_FRAME_LED_SIZE ; i++)
  {
    ...
    for (uint8_t j = 0 ; j < WS2812_LED_RGB_SIZE ; j++)
    {
      ...
      {
        *(frameToUpdate + i * WS2812_LED_SIZE + j * 8 + k ) = (pixelComponents[j] & 0x80) ? WS2812_HIGH_DUTY : WS2812_LOW_DUTY;
        pixelComponents[j] <<= 1;
      }
    }
  }
