...
bool getCardNumber(uint32_t id[])
{
  if(!error)
  {
    ...
    for(i = 0; i < 40; i++)
    {
      id[i] = data[i * BITS_PER_CHAR + 0] + (data[i * BITS_PER_CHAR + 1] << 1) + (data[i * BITS_PER_CHAR + 2] << 2) + (data[i * BITS_PER_CHAR + 3] << 3);
    }
    id[]
  }
  return !error;
...
static void dataCheck(void)
{
    ...
    for(i = 0; i < 40; i++)
    {
      for(j = 0; j < 4; i++)
        {
          sum = (sum + data[i * BITS_PER_CHAR + j]);
        }
      if((sum & (0x01)) == data[i * BITS_PER_CHAR + 4])			// Looking for the bit nº4 which is parity bit
              {
                error = true;			
              }
      sum = 0;
